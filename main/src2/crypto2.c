...
{
  switch (status) {
    case PSA_ERROR_INVALID_ARGUMENT: return CRYPTO_ERR_INVALID_ARGS;
    case PSA_SUCCESS:                return CRYPTO_SUCCESS;
    case PSA_ERROR_INVALID_HANDLE:   return CRYPTO_ERR_INVALID_HANDLE;
  ...
  crypto_key_t *private_key)
{
  ...
  status = psa_key_derivation_input_bytes(
    &derivation, PSA_KEY_DERIVATION_INPUT_SALT, NULL, 0);
  if (status != PSA_SUCCESS) { return psa_status_to_crypto(status); }

  status = psa_key_derivation_input_bytes(
    &derivation, PSA_KEY_DERIVATION_INPUT_SECRET, secret->raw.data, 32);
  if (status != PSA_SUCCESS) { return psa_status_to_crypto(status); }

  status = psa_key_derivation_input_bytes(
    &derivation, PSA_KEY_DERIVATION_INPUT_INFO, &counter, sizeof(counter));
  if (status != PSA_SUCCESS) { return psa_status_to_crypto(status); }

  status = psa_key_derivation_output_bytes(&derivation, ephemeral_private, 32);
  if (status != PSA_SUCCESS) { return psa_status_to_crypto(status); }
  ...
  status = psa_import_key(&attr, ephemeral_private, 32, &private_key->id);
  if (status != PSA_SUCCESS) { return status; }
  ...
  crypto_key_t *aes_key)
{
  ...
  status = psa_key_derivation_input_bytes(
    &deriv, PSA_KEY_DERIVATION_INPUT_SECRET, secret->raw.data, 32);
  if (status != PSA_SUCCESS) { goto cleanup; }

  status = psa_key_derivation_input_bytes(
    &deriv, PSA_KEY_DERIVATION_INPUT_INFO, info, info_len);
  if (status != PSA_SUCCESS) { goto cleanup; }

  uint8_t finder_sym_bytes[32];

  status = psa_key_derivation_output_bytes(&deriv, finder_sym_bytes, 32);
  if (status != PSA_SUCCESS) { goto cleanup; }
...
crypto_status_t generate_ecdsa_keypair(crypto_key_t *keypair)
{
  ...
  psa_status_t status = psa_generate_key(&attr, &keypair->id);
  keypair->type = KEY_TYPE_ID;
  ...
  crypto_key_t *key_out)
{
  ...
  psa_set_key_bits(&attr, 256);
  psa_set_key_usage_flags(&attr, PSA_KEY_USAGE_VERIFY_MESSAGE | PSA_KEY_USAGE_VERIFY_HASH);
  psa_set_key_algorithm(&attr, PSA_ALG_ECDSA(PSA_ALG_SHA_256));
